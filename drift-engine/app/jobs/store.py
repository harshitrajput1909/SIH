"""In-memory job store with cooperative cancellation."""
...
from app.schemas import JobStatus, JobStatusResponse, ShiftAssessmentResult
...
class JobRecord:
    def __init__(self, job_id: str) -> None:
        ...
        self.result: ShiftAssessmentResult | None = None
    ...
    def complete(self, result: ShiftAssessmentResult) -> None:
        with self._lock:
...
class JobStore:
    ...
    def create(self) -> JobRecord:
        record = JobRecord(f"tvsd-{uuid.uuid4().hex[:12]}")
        with self._lock:
