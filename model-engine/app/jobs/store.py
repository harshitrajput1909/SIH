"""In-memory job store (single-deployment view; durable state lives upstream)."""
...
from app.schemas import JobStatus, ModelAssessmentResult, JobStatusResponse
...
class JobRecord:
    def __init__(self, job_id: str) -> None:
        ...
        self.result: ModelAssessmentResult | None = None
    ...
    def complete(self, result: ModelAssessmentResult) -> None:
        with self._lock:
...
class JobStore:
    ...
    def create(self) -> JobRecord:
        record = JobRecord(f"tvmm-{uuid.uuid4().hex[:12]}")
        with self._lock:
